...
class QueryExpander:
    ...
    DOMAIN_SYNONYMS = {
        ...
        "energy": {
            ...
            "en": ["energy", "power", "electricity", "consumption", "production", "solar"]
        },
        "security": {
    ...
    REFORMULATION_PATTERNS = [
        ...
        (r"(hány|mennyi)\s+fok\s+van\s+(a|az)\s+(\w+)", r"what is the temperature in the \3"),
    ...
    async def expand_query(
        ...
        try:
            ...
            processing_time = max(1, int((datetime.now() - start_time).total_seconds() * 1000))
            
            return QueryExpansionResult(
                original_query=original_query,
                expanded_queries=selected_variants,
                expansion_methods=[c.method for c in filtered_candidates[:len(selected_variants)]],
                confidence_scores=[c.confidence for c in filtered_candidates[:len(selected_variants)]],
    ...
    async def _generate_translation_variants(self, query: str) -> List[ExpansionCandidate]:
    ...
    async def _generate_reformulation_variants(self, query: str) -> List[ExpansionCandidate]:
    ...
    def _filter_and_rank_candidates(
        ...
        for candidate in unique_candidates:
            ...
            if self._calculate_similarity(original_query.lower(), candidate.query) > 0.95:
                continue
    ...
    def _select_top_variants(
        ...
        for candidate in candidates:
            ...
            if candidate.method != "original":  # Skip original since it's already included
