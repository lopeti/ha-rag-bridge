...
async def add_edge(edge: schemas.EdgeCreate, request: Request) -> schemas.EdgeResult:
    ...
    db = arango.db(
        db_name, username=os.environ["ARANGO_USER"], password=os.environ["ARANGO_PASS"]
    )
    ...
    edge_key = (
        f"{edge.from_.replace('/', '_')}-{edge.to.replace('/', '_')}-{edge.label}"
    )
    edge_doc = {
        "_key": edge_key,
        "_from": edge.from_,
    ...
    from app.schemas import EdgeResult

    return EdgeResult(status="ok", edge_key=edge_key, action=action)
