...
class FriendlyNameGenerator:
    ...
    def __init__(self):
        ...
        self.domain_patterns = {
            ...
            'cover': {
                'default_suffix': 'redőny'
            },
            'switch': {
                'default_suffix': 'kapcsoló'
            },
            'climate': {
                'default_suffix': 'klíma'
            },
            'media_player': {
                'default_suffix': 'médialejátszó'
            },
            'weather': {
                'default_suffix': 'időjárás'
            }
        }
    ...
    def _build_name(self, name_part: str, domain: str, area_id: Optional[str], 
                    device_class: Optional[str], unit_of_measurement: str) -> Tuple[str, float, str]:
        ...
        for word in remaining_words:
            ...
            if word == 'pm' and len(remaining_words) >= 3 and remaining_words[remaining_words.index(word)+1:remaining_words.index(word)+3] == ['2', '5']:
        ...
        if domain in self.domain_patterns:
            ...
            if 'measurement_patterns' in domain_config and device_class and unit_of_measurement:
                pattern_key = (device_class.lower(), unit_of_measurement.lower())
                if pattern_key in domain_config['measurement_patterns']:
                    measurement_name = domain_config['measurement_patterns'][pattern_key]
                    ...
                    confidence += 0.15
                    reasoning_parts.append(f'mérési minta: {pattern_key}→{measurement_name}')
            ...
            elif domain_config.get('default_suffix') and not any(reasoning for reasoning in reasoning_parts if 'minta' in reasoning or 'device_class' in reasoning):
                name_components.append(domain_config['default_suffix'])
                confidence += 0.05
        ...
        reasoning = '; '.join(reasoning_parts) if reasoning_parts else 'alapértelmezett név generálás'
    ...
    def filter_suggestions(self, suggestions: List[FriendlyNameSuggestion], 
                          min_confidence: float = 0.5,
                          domains: Optional[List[str]] = None) -> List[FriendlyNameSuggestion]:
...
def main():
    ...
    if args.test:
        ...
        test_entities = [
            ...
            {'entity_id': 'binary_sensor.konyha_mozgaserzekelo_occupancy', 'area_id': 'konyha', 'device_class': 'occupancy'},
            {'entity_id': 'sensor.energy_production_today', 'unit_of_measurement': 'kWh', 'device_class': 'energy'},
            {'entity_id': 'cover.bubi_redony', 'area_id': 'bubi'},
            {'entity_id': 'sensor.nappali_homerseklet', 'area_id': 'nappali', 'device_class': 'temperature', 'unit_of_measurement': '°C'},
