...
class HAConfigAdvisor:
            ...
            if len(group) > 1:
                ...
                if len(set(areas)) > 1:
                    ...
                    issues.append(ConfigIssue(
                        ...
                        auto_fixable=False,
                        metadata={"entities": entity_names, "areas": list(set(areas))}
                    ))
    ...
    def _find_similar_entities_across_areas(self) -> List[List[dict]]:
        ...
        for entity in self.entities:
            domain = entity.get("entity_id", "").split(".")[0] if entity.get("entity_id") else ""
            device_class = entity.get("device_class", "")
    ...
    def _generate_recommendations(self, issues: List[ConfigIssue]) -> List[str]:
        ...
        orphaned_entities = len([i for i in issues if i.category == IssueCategory.ENTITY_ORPHANED])
        orphaned_devices = len([i for i in issues if i.category == IssueCategory.DEVICE_AREA])
        naming_issues = len([i for i in issues if i.category in [IssueCategory.FRIENDLY_NAME, IssueCategory.DEVICE_NAMING]])
        
        if orphaned_entities > 0:
            recommendations.append(f"Assign areas to {orphaned_entities} orphaned entities for better organization")
        
        if orphaned_devices > 0:
            recommendations.append(f"Assign areas to {orphaned_devices} devices to improve entity grouping")
            
        if naming_issues > 5:
            recommendations.append("Consider reviewing entity and device names for better clarity")
            
        if len(self.areas) < 5 and len(self.entities) > 20:
            recommendations.append("Consider creating more areas to better organize your entities")
        
        recommendations.append("Run the advisor regularly to maintain optimal configuration")
    ...
    def display_report(self, report: AnalysisReport, show_details: bool = True) -> None:
        ...
        for level in IssueLevel:
            ...
            if count > 0:
                color = {"critical": "red", "error": "red", "warning": "yellow", "info": "blue"}.get(level.value, "white")
                summary_table.add_row(f"{level.value.title()} Issues", str(count), style=color)
        ...
        if show_details and report.issues:
            ...
            for category, issues in issues_by_category.items():
                ...
                for issue in issues[:5]:  # Show first 5 issues per category
                    level_color = {"critical": "red", "error": "red", "warning": "yellow", "info": "blue"}[issue.level.value]
                ...
                if len(issues) > 5:
                    console.print(f"  ... and {len(issues) - 5} more issues in this category")
        
        console.print(f"[dim]Report generated at: {report.generated_at.strftime('%Y-%m-%d %H:%M:%S')}[/dim]")
    
    def generate_friendly_name_suggestions(self, min_confidence: float = 0.7) -> List[Dict]:
    ...
    def apply_friendly_name_suggestions(self, suggestions: List[Dict], 
        ...
        updates = [
            {
                ...
                "friendly_name": s["suggested_name"]
            }
        ...
        try:
            with httpx.Client(
                ...
                response = client.post(
                    "/api/rag/batch_update_friendly_names",
...
def main():
    ...
    parser.add_argument(
        "--detailed", 
        action="store_true",
        help="Show detailed issue breakdown"
    )
    parser.add_argument(
        "--category",
    ...
    try:
        ...
        if args.suggest_friendly_names or args.apply_friendly_names:
            suggestions = advisor.generate_friendly_name_suggestions(min_confidence=args.confidence)
            
            if args.suggest_friendly_names:
                console.print("[bold green]🧠 Friendly Name Suggestions[/bold green]")
                
                if not suggestions:
                    console.print("No suggestions found with the specified confidence threshold.")
                    return 0
                
                table = Table(title=f"Friendly Name Suggestions (confidence >= {args.confidence})")
            ...
            if args.apply_friendly_names:
                console.print("[bold yellow]🔄 Applying Friendly Name Suggestions[/bold yellow]")
                ...
                if result.get("dry_run"):
                    console.print(f"[blue]DRY RUN: Would update {result['would_update']} entities[/blue]")
                    return 0
                
                if result.get("success"):
                    console.print(f"[green]✅ Successfully updated {result['updated']} entities![/green]")
