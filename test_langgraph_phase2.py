...
async def test_full_workflow_integration():
    ...
    for i, test_case in enumerate(test_cases, 1):
        ...
        try:
            ...
            scope_str = detected_scope.value if hasattr(detected_scope, "value") else str(detected_scope)
            ...
            print(f"   Retrieved entities: {len(retrieved_entities)} total, {len(cluster_entities)} from clusters")
            ...
            k_in_range = test_case["expected_k_range"][0] <= optimal_k <= test_case["expected_k_range"][1]
            ...
            overall_success = scope_correct and k_in_range and has_entities and has_context and not errors
            ...
            print(f"   ❌ ERROR: {e}")
            logger.error(f"Test failed for query '{test_case['query']}': {e}", exc_info=True)
            results.append({
                "query": test_case["query"],
                "success": False,
                "error": str(e),
    ...
    print(f"Target: 100% (Phase 2 should handle all core cases)")
    ...
    for r in results:
        if r.get("success", False):
            entities_info = f"({r['cluster_entity_count']}/{r['entity_count']} from clusters)"
            print(f"✅ {r['query'][:40]:<40} → {r['actual_scope']:<8} | {r['actual_formatter']:<15} | {r['entity_count']} entities {entities_info}")
        else:
...
async def test_conversation_flow():
    ...
    print(f"   Areas: {result1.get('conversation_context', {}).get('areas_mentioned', [])}")
    
    # Second query: follow-up 
    print("\n2. Follow-up query:")
    conversation_history = [
        {"role": "user", "content": "mi van a nappaliban?"},
        {"role": "assistant", "content": "A nappaliban 22°C van, a lámpák ki vannak kapcsolva."},
    ]
    ...
    print(f"   Areas: {result2.get('conversation_context', {}).get('areas_mentioned', [])}")
    print(f"   Is follow-up: {result2.get('conversation_context', {}).get('is_follow_up', False)}")
...
async def test_cluster_effectiveness():
    ...
    for query in cluster_test_queries:
        ...
        retrieved_entities = result.get("retrieved_entities", [])
        cluster_entities = result.get("cluster_entities", [])
        cluster_ratio = len(cluster_entities) / len(retrieved_entities) if retrieved_entities else 0
