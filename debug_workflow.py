    ...
    try:
        ...
        sections = {
            ...
            "🔍 Entity Retrieval": {
                ...
                "memory_boosted": len([e for e in result.get("retrieved_entities", []) if e.get("_memory_boosted")])
            },
            "📝 Context Formatting": {
                "formatter_type": result.get("formatter_type", "unknown"),
                "context_length": len(result.get("formatted_context", "")),
                "context_preview": result.get("formatted_context", "")[:200] + "..." if len(result.get("formatted_context", "")) > 200 else result.get("formatted_context", "")
            },
            "⚠️  Errors & Retries": {
        ...
        if entities:
            ...
            for i, entity in enumerate(entities, 1):
                ...
                print(f"  {i}. {entity_id} (score: {score:.3f}) - {area}{memory_boost}{cluster_context}")
        ...
        if diagnostics:
            ...
            component_scores = {
                "Conversation Analysis": diagnostics.get("conversation_analysis_quality", 0.0),
                "Scope Detection": diagnostics.get("scope_detection_quality", 0.0), 
                "Entity Retrieval": diagnostics.get("entity_retrieval_quality", 0.0),
                "Context Formatting": diagnostics.get("context_formatting_quality", 0.0)
            }
