...
async def debug_workflow_clean(query: str, session_id: str = "debug_session"):
    ...
    try:
        ...
        print("🗣️  Conversation Analysis:")
        print(f"    Areas: {', '.join(conv_ctx.get('areas_mentioned', []))}")
        print(f"    Domains: {', '.join(conv_ctx.get('domains_mentioned', []))}")
        print(
            f"    Intent: {conv_ctx.get('intent', 'unknown')} (conf: {conv_ctx.get('confidence', 0):.1f})"
        )
        ...
        print(
            f"    Clusters: {len(cluster_entities)} | Memory: {len(memory_entities)} | Boosted: {len(memory_boosted)}"
        )
        ...
        primary_count = (
            primary_line.count(" | ") + 1 if primary_line and ":" in primary_line else 0
        )
        related_count = (
            related_line.count(" | ") + 1 if related_line and ":" in related_line else 0
        )
        total_injected = primary_count + related_count

        print(f"\n📝 Context Formatting: {formatter_type} ({context_len} chars)")
        print(
            f"    Entities in final prompt: {total_injected} ({primary_count} primary + {related_count} related)"
        )
        ...
        relevant_entities = [
            e
            for e in entities[:8]
            if e.get("_score", 0) > 0.1 or e.get("_memory_boosted")
        ]
        ...
        if relevant_entities:
            for i, entity in enumerate(relevant_entities, 1):
                ...
                try:
                    ...
                    clean_name = entity_reranker.SystemPromptFormatter._get_clean_name(
                        entity
                    )
                except Exception:
                    clean_name = "N/A"
                ...
                print(
                    f"        Score: {score:.3f} | Area: {area} | Clean Name: '{clean_name}'{badge_str}"
                )
        else:
        ...
        if errors or diagnostics:
            ...
            if diagnostics:
                ...
                component_str = " | ".join(
                    [f"{k}:{v:.1f}" for k, v in components.items()]
                )
