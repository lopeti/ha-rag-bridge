...
def test_manual_hints(monkeypatch):
    ...
    monkeypatch.setattr(main, "retrieve_entities", MagicMock(return_value=[{"entity_id": "light.test", "domain": "light", "device_id": "dev1"}]))

    monkeypatch.setattr(main, "query_manual", MagicMock(return_value=["hint1", "hint2"]))
